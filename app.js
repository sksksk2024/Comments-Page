...
const createCommentNode = (commentObject) => {
  ...
  // Attach event listeners for score buttons
  commentNode.querySelector(".score-plus").addEventListener("click", () => {
    commentObject.score++;
    initComments(); // Re-render the comments
  });
  ...
  if (commentObject.user.username === data.currentUser.username) {
    ...
    commentNode.querySelector(".edit").addEventListener("click", (e) => {
      const commentTextElement = e.currentTarget.closest('.comment').querySelector(".c-body");

      if (commentTextElement) {
        const isEditable = commentTextElement.getAttribute("contenteditable") === "true";
        
        if (!isEditable) {
          console.log("Enabling editing for:", commentTextElement.textContent);
          commentTextElement.setAttribute("contenteditable", "true");
          commentTextElement.focus(); // Focus on the element for immediate editing
          e.currentTarget.textContent = "Save"; // Optionally, change the button text to "Save"
        } else {
          console.log("Disabling editing for:", commentTextElement.textContent);
          commentTextElement.setAttribute("contenteditable", "false");
          e.currentTarget.textContent = "Edit"; // Optionally, revert the button text to "Edit"
          // Here, you might want to save the edited content back to the `commentObject`
          commentObject.content = commentTextElement.textContent;
        }
      } else {
        console.log("Could not find the .c-body element.");
      }
    });
  }
